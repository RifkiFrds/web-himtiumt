export const tabsData = [
  { title: "Semua", value: "semua" },
  { title: "Event", value: "event" },
  { title: "Edukasi", value: "edukasi" },
  { title: "Informasi", value: "informasi" },
  { title: "Teknologi", value: "teknologi" },
];

export const dataArtikel = [
  // --- EVENT ---
  {
    id: 1,
    title: "Workshop UI/UX Design",
    category: "event",
    description:
      "Merancang antarmuka yang berpusat pada pengguna untuk pengalaman digital yang lebih baik.",
    longDescription:
      "Deskripsi panjang workshop UI/UX, mencakup konsep desain, tools, dan praktik terbaik antarmuka.",
    header: "/images/prestasi/lomba-uiux.jpg",
    className: "md:col-span-2",
  },
  {
    id: 2,
    title: "Webinar Cybersecurity",
    category: "event",
    description:
      "Melindungi data dan privasi di era digital dari ancaman siber yang terus berkembang.",
    longDescription:
      "Deskripsi panjang webinar cybersecurity, termasuk tips keamanan digital dan praktik terbaik.",
    header: "/images/program/cyber.jpg",
    className: "md:col-span-1",
  },
  {
    id: 3,
    title: "Persiapan Menghadapi Kompetisi Programming",
    category: "event",
    description:
      "Tips dan trik dari para juara untuk mengasah kemampuan problem-solving.",
    longDescription:
      "Deskripsi panjang event kompetisi programming, termasuk latihan, strategi, dan pengalaman peserta sebelumnya.",
    header: "/images/prestasi/lomba-hackathon.jpg",
    className: "md:col-span-2",
  },

  // --- EDUKASI ---
  {
    id: 4,
    title: "Metode STAR untuk Pengembangan Diri Mahasiswa",
    category: "edukasi",
    description:
      "Pemahaman tentang ekonomi digital dalam era revolusi industri 4.0.",
    longDescription:
      "Penjelasan lengkap penggunaan metode STAR, dengan contoh aplikasi dalam kehidupan sehari-hari mahasiswa.",
    header: "/images/edukasi/metode-star.jpg",
    className: "md:col-span-1",
  },
  {
    id: 5,
    title: "Revolusi Industri 4.0 dan Ekonomi Digital",
    category: "edukasi",
    description:
      "Pemahaman tentang ekonomi digital dalam era revolusi industri 4.0.",
    longDescription:
      "Artikel edukasi tentang pengaruh revolusi industri 4.0, peran e-commerce, fintech, dan digital marketing.",
    header: "/images/edukasi/ekonomi-digital.jpg",
    className: "md:col-span-1",
  },

  // --- INFORMASI ---
  {
    id: 6,
    title: "Pengumuman Jadwal Akademik Semester Ganjil",
    category: "informasi",
    description:
      "Update terbaru mengenai kalender akademik dan kegiatan kampus untuk semester ganjil.",
    longDescription:
      "Detail jadwal akademik, tanggal penting, dan kegiatan mahasiswa di semester ganjil.",
    header: "/images/program/jadwal-akademik.jpg",
    className: "md:col-span-2",
  },
  {
    id: 7,
    title: "Prestasi Mahasiswa HIMTI dalam Kompetisi Nasional",
    category: "informasi",
    description:
      "Highlight prestasi dan penghargaan mahasiswa dalam kompetisi tingkat nasional.",
    longDescription:
      "Rangkuman prestasi mahasiswa, lomba yang diikuti, serta tips sukses dari para pemenang.",
    header: "/images/prestasi/prestasi-mahasiswa.jpg",
    className: "md:col-span-1",
  },

  // --- TEKNOLOGI (3 artikel) ---
  {
    id: 8,
    title: "10 Tren Teknologi di 2025",
    category: "teknologi",
    description: "Mengenal tren teknologi yang akan mendominasi di tahun 2025.",
    longDescription:
      "Sepuluh tren teknologi yang diprediksi berkembang pesat, termasuk AI, IoT, dan blockchain.",
    header: "/images/teknologi/tren-teknologi-2025.jpg",
    className: "md:col-span-2",
  },
  {
    id: 9,
    title: "Inovasi Teknologi yang Mengubah Cara Hidup",
    category: "teknologi",
    description:
      "Inovasi-inovasi teknologi yang berpotensi mengubah gaya hidup manusia.",
    longDescription:
      "Delapan inovasi teknologi yang merevolusi cara kita bekerja, belajar, dan berinteraksi.",
    header: "/images/teknologi/inovasi-teknologi.jpg",
    className: "md:col-span-1",
  },
  {
    id: 10,
    title: "AI Bisa Merusak Otak Kita, Kok Bisa?",
    category: "teknologi",
    description:
      "Bahaya penggunaan AI yang tidak terkendali terhadap kesehatan mental.",
    longDescription:
      "Dampak negatif penggunaan AI berlebihan terhadap kesehatan otak manusia, termasuk gangguan tidur dan konsentrasi.",
    header: "/images/teknologi/ai-dan-kesehatan.jpg",
    className: "md:col-span-1",
  },
];
